...
def send_order_confirmation(self, order_id: int):
    ...
    try:
        ...
        # Note: In production, we should handle the async call properly. 
        # For simplicity here, we are using a helper or assuming we can run it.
        # Since Celery is sync by default, we need to run the async function.
